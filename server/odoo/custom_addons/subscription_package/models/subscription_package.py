...
class SubscriptionPackage(models.Model):
    ...
    member_id = fields.Many2one('sffla_ordinary.res_members', string="Membership")
    ...
    def _onchange_plan_id(self):
        """ When selecting a plan, replace existing product lines with the plan's default products """
        if self.plan_id:
            # Clear existing product lines
            self.product_line_ids = [(5, 0, 0)]

            # Add new lines from selected plan
            lines = []
            for plan_line in self.plan_id.product_line_ids:
                lines.append((0, 0, {
                    'product_id': plan_line.product_id.id,
                    'product_qty': plan_line.product_qty,
                    'unit_price': plan_line.unit_price,
                    'discount': plan_line.discount,
                    'tax_ids': [(6, 0, plan_line.tax_ids.ids)],
                }))
    ...
    def button_start_date(self):
        ...
        for rec in self:
            # Ensure only one "Approved" stage exists
            if len(self.env['subscription.package.stage'].search(
                    [('category', '=', 'approved')])) > 1:
                raise UserError(
                    _('More than one stage is having category "Approved". '
                      'Please ensure only one stage has category "Approved".'))

            if not rec.product_line_ids:
                raise UserError("Empty order lines !! Please add the subscription product.")

            # --- Your existing sale order logic ---
            if rec.sale_order_id:
                ...
                for line in rec.sale_order_id.order_line.filtered(
                        lambda x: x.product_template_id.is_subscription):
                    line.qty_to_invoice = line.product_uom_qty

            # --- NEW: generate a membership ID ---
            # Only create if this partner does not already have one
            if not self.env['res.members'].search(
                    [('partner_id', '=', rec.partner_id.id)], limit=1):
                new_member = self.env['res.members'].create({
                    'partner_id': rec.partner_id.id,
                    # you can add join_year or other fields if needed
                    'join_year': str(fields.Date.today().year),
                    'membership_type': rec.plan_id.id,
                })
                # optional: keep a link back to the created membership
                rec.member_id = new_member.id
